...
# ----------------------------------------------------------------------
# 1. Настройка шрифта для кириллицы (как в предыдущем коде)
# ----------------------------------------------------------------------
...
def latex_to_image(latex_expr, dpi=100, fontsize=14):
    ...
    plt.rc('text', usetex=False)          # не используем внешний LaTeX (чтобы не требовать его установки)
    plt.rc('mathtext', fontset='stix')    # шрифт для формул
    ...
    img = Image(tmp_path)
    # Настроим размеры так, чтобы формула была читаемой
    img.drawHeight = img.drawHeight / 2.0   # уменьшаем высоту (подберите под себя)
    img.drawWidth = img.drawWidth / 2.0
    img.hAlign = 'LEFT'
    # Удалим временный файл после использования? ReportLab читает файл сразу,
    # поэтому можно удалить только после того, как мы передали img в story.
    # Для простоты оставим файл, он удалится при завершении скрипта.
    # Но можно использовать менеджер контекста, сложно. Оставим как есть.
    # В конце можно удалить вручную.
    return img, tmp_path
...
def split_text_and_formulas(text):
    ...
    parts = []
    pos = 0
    # Ищем все вхождения $...$ (не учитывая экранирование)
    pattern = r'\$([^\$]+)\$'
...
def parse_content_to_flowables(content, base_style):
    ...
    temp_files = []  # для последующей очистки
    for typ, value in parts:
        if typ == 'text':
            ...
            elements.append(img)
            # после изображения можно добавить небольшой пробел
...
def json_to_pdf_questions_only(json_path, pdf_path):
    ...
    question_style = ParagraphStyle('Question', parent=base_style, fontSize=12, leading=16,
                                     spaceAfter=6, spaceBefore=12, fontWeight='bold')
    ...
    if test_title:
        title_style = ParagraphStyle('Title', parent=base_style, fontSize=16,
                                      spaceAfter=12, fontWeight='bold')
        story.append(Paragraph(test_title, title_style))
        story.append(Spacer(1, 0.5*cm))

    all_temp_files = []  # список временных файлов для удаления
