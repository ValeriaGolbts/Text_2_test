...
# ----------------------------------------------------------------------
def latex_to_image(latex_expr, fontsize=16, max_width_cm=14):
    """
    Преобразует строку с LaTeX в PNG и возвращает путь к файлу.
    Улучшенная версия с правильным масштабированием.
    """
    ...
    # Устанавливаем максимальную высоту для inline формул
    max_height_pt = 20  # примерно соответствует размеру шрифта 12pt
...
# ----------------------------------------------------------------------
def create_inline_paragraph(content, style, base_font_size=12):
    """
    ...
    for typ, value in parts:
        if typ == 'text':
            ...
            try:
                ...
                img, tmp_path = latex_to_image(value, fontsize=base_font_size + 2)
...
def json_to_pdf_questions_only(json_path, pdf_path):
    ...
    text_style = ParagraphStyle(
        ...
        fontName=font_name,
        fontSize=12, 
        leading=20,  # Увеличенный межстрочный интервал для формул
        encoding='utf-8',
        spaceBefore=2,
        spaceAfter=2
    )
