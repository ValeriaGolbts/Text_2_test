...
class LLMExpertEvaluator:
    ...
    def __init__(self, llm_client, temperature: float = 0.2):
        ...
        self.criteria = {
            "accuracy": {
                ...
                "scale": "1=много ошибок, 5=полностью корректно"
            },
            "grounding": {
                "name": "Соответствие материалу",
                "description": "Можно ли найти ответы в исходной лекции?",
                "scale": "1=ответов нет в тексте, 5=все ответы есть в тексте"
            },
            "meaningfulness": {
                "name": "Осмысленность",
            ...
            "diversity": {
                ...
                "scale": "1=все вопросы об одном, 5=хорошее покрытие тем"
            },
            "usefulness": {
                "name": "Полезность",
                "description": "Насколько тест полезен для проверки знаний?",
                "scale": "1=бесполезен, 5=очень полезен"
            }
        }
    ...
    def _create_judge_prompt(self, chunks_text: str, test_text: str) -> str:
        ...
        return f"""Ты — эксперт по педагогике и методике преподавания точных наук (математика, физика). Оцени качество теста, который был сгенерирован по фрагменту лекции.
    ...
    async def evaluate_with_consistency_check(self,
        ...
        if verbose:
            ...
            if avg_std <= 0.5:
                ...
                print(f"   ⚠️ LLM умеренно стабилен")
            else:
