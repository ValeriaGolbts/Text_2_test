...
class PipelineJSONProcessor:
    ...
    def select_chunks_80_percent(self, pipeline_data: Dict[str, Any]) -> Tuple[List[Dict], Dict]:
        ...
        4. Отбирает самые информативные чанки с учётом эффективности (важность/токен)
        """
        ...
        # === ШАГ 5: Сортировка по эффективности (важность/токен) ===
        chunks_scored = []
        for i, (chunk, tokens) in enumerate(zip(chunks, token_counts)):
            ...
            # Рассчитываем эффективность: важность на 1000 токенов
            efficiency = (importance / max(1, tokens)) * 1000
        ...
        # Сортировка по эффективности (самые эффективные первые)
        chunks_scored.sort(key=lambda x: x["efficiency"], reverse=True)
        ...
        print(f"\nОтбор {target_chunks} лучших по эффективности:")
        
        # Сначала пытаемся набрать target_chunks чанков
        for i, item in enumerate(chunks_scored):
            if len(selected) >= target_chunks:
                break
            
            if selected_tokens + item["tokens"] > available_tokens:
                # Если не влезает по токенам, пробуем следующий чанк
                continue
        ...
        # Если не удалось набрать target_chunks, пробуем добирать менее эффективными
        if len(selected) < target_chunks:
            print(f"  Не удалось набрать {target_chunks} чанков, добираем из оставшихся")
            remaining = [item for item in chunks_scored if item["chunk"] not in selected]
        ...
        selection_info = {
            ...
            "strategy": "80_percent_of_max_possible_with_efficiency"
        }
    
        print(f"\nРезультат отбора:")
        print(f"  Отобрано: {len(selected)} из {max_chunks_real} возможных")
        print(f"  Целевое количество (80%): {target_chunks} чанков")
        ...
        if len(selected) < target_chunks:
            print(f"  Предупреждение: отобрано меньше целевого количества из-за лимита токенов")
...
async def main():
    ...
    if 'error' not in result:
        ...
        if selection_info:
            ...
            print(f"    Целевое количество (80%): {selection_info['target_chunks_80pct']} чанков")
