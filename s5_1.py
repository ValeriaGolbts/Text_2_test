...
МЕТОДОЛОГИЯ: CoT (пошаговость) + Few-Shot (примеры) + Constraints (жесткие ограничения)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ЖЕСТКИЕ ОГРАНИЧЕНИЯ (Constraints - НАРУШАТЬ НЕЛЬЗЯ):

Ограничение 1: ТОЧНОЕ количество вопросов = {num_questions}
Ограничение 2: У каждого вопроса РОВНО 4 варианта ответа
Ограничение 3: ТОЛЬКО 1 правильный ответ на вопрос
Ограничение 4: Ответы НЕ могут быть "все вышеперечисленное" или "А и Б"
Ограничение 5: Каждый вопрос ДОЛЖЕН ссылаться на конкретное место в материале
Ограничение 6: Минимум 30% вопросов должны содержать формулы или термины
Ограничение 7: Длина explanation от 20 до 150 символов

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ЭТАЛОННЫЙ ПРИМЕР (Few-Shot - следуй этому качеству):

МАТЕРИАЛ: "Согласно второму закону Ньютона, ускорение тела прямо пропорционально силе и обратно пропорционально массе: a = F/m"

ИДЕАЛЬНЫЙ ВОПРОС:
{{
    "id": 1,
    "question": "Если на тело массой 2 кг действует сила 10 Н, какое ускорение приобретет тело?",
    "options": ["2 м/с²", "5 м/с²", "10 м/с²", "20 м/с²"],
    "correct_answer": "5 м/с²",
    "explanation": "По формуле a = F/m = 10 Н / 2 кг = 5 м/с²",
    "material_reference": "второй закон Ньютона, a = F/m"
}}
...
ПОШАГОВЫЙ ПРОЦЕСС (CoT - выполни последовательно):

<step1>
Прочитай материал. Выпиши:
- Все формулы (если есть)
...
<step2>
На основе выписанного создай план {num_questions} вопросов.
Убедись, что выполняется Ограничение 6 (30% с формулами).
Запиши план в виде: Вопрос N: тема, тип, сложность
</step2>

<step3>
Для каждого пункта плана создай вопрос по образу эталонного примера.
Проверь каждое Ограничение 1-7.
</step3>

<step4>
Финальная проверка:
- Подсчитай вопросы (Огр.1) ✓/✗
- Проверь варианты (Огр.2) ✓/✗
- Проверь правильность (Огр.3) ✓/✗
- Проверь форматы ответов (Огр.4) ✓/✗
- Есть ли ссылки на материал (Огр.5) ✓/✗
- Формулы/термины (Огр.6) ✓/✗
- Длина explanation (Огр.7) ✓/✗
</step4>
...
{{
    "test_title": "...",
    "generation_method": "Chain_FewShot_Constrained",
    "constraints_validation": {{
        "exact_question_count": {num_questions},
        "four_options_each": true,
        "single_correct_answer": true,
        "no_forbidden_formats": true,
        "material_references": true,
        "formulas_coverage": ">=30%",
    ...
    "questions": [
        {{
            ...
            "correct_answer": "...",
            "explanation": "...",
            "material_reference": "конкретное место в материале"
        }}
    ]
}}
"""
