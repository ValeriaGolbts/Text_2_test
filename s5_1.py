...
"""
        ...
        prompt = f"""ТВОЯ ЗАДАЧА: Создать тест из {num_questions} вопросов на основе учебного материала.

КЛЮЧЕВОЕ ТРЕБОВАНИЕ: РАЗМЫШЛЯЙ ПОШАГОВО. Каждый шаг пиши в <thought> тегах, затем выполняй действие.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

<step1_analysis>
<thought>
ШАГ 1: Анализ материала. Нужно выделить:
- Основные темы (из {topics_str})
- Ключевые термины и определения
- Формулы и их взаимосвязи
...
<action1>
Выполни анализ. Запиши найденное в виде списка ключевых элементов.
</action1>
</step1_analysis>
...
<step2_planning>
<thought>
ШАГ 2: Планирование вопросов. На основе анализа нужно:
- Распределить {num_questions} вопросов по темам
- Определить сложность каждого вопроса (easy/medium/hard)
- Выбрать тип вопроса (определение/применение/анализ)
- Убедиться, что каждая тема покрыта хотя бы одним вопросом
</thought>

<action2>
Создай план теста. Укажи для каждого вопроса: тему, сложность, тип.
</action2>
</step2_planning>
...
<step3_generation>
<thought>
ШАГ 3: Генерация вопросов. Для каждого пункта плана:
1. Сформулируй четкий вопрос (не дословно из материала)
2. Придумай 4 варианта ответа (1 правильный, 3 правдоподобных)
3. Правильный ответ должен быть лучшим, но не очевидным
4. Напиши пояснение, почему ответ правильный
</thought>

<action3>
Сгенерируй все {num_questions} вопросов по плану.
</action3>
</step3_generation>
...
<step4_validation>
<thought>
ШАГ 4: Проверка качества. Каждый вопрос должен:
- Иметь ровно 4 варианта
- Содержать только 1 правильный ответ
- Быть основан строго на материале
- Не содержать подсказок в вопросе
</thought>

<action4>
Проверь все вопросы. Исправь найденные проблемы.
</action4>
</step4_validation>
...
<final_output>
Теперь, после всех размышлений, выведи ТОЛЬКО JSON с готовым тестом:

{{
    "test_title": "Название, отражающее основные темы",
