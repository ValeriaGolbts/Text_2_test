...
"""
        ...
        prompt = f""" ТВОЯ ЗАДАЧА: Создать тест из {num_questions} вопросов на основе РАСШИФРОВКИ АУДИОЛЕКЦИИ.

ВАЖНО: Расшифровка может содержать речевые обороты. Твоя задача — НОРМАЛИЗОВАТЬ их в академические формулировки.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📚 ПРИМЕР №1 (Как преобразовать речь в вопрос):

❌ ФРАЗА ЛЕКТОРА В РАСШИФРОВКЕ:
"Ну, короче, эээ, первый закон термодинамики — это, как бы, энергия не берется из ниоткуда и не исчезает в никуда, она просто переходит из одного вида в другой, понимаете?"

✅ НОРМАЛИЗОВАННЫЙ ВОПРОС:
{{
    "question": "Согласно первому закону термодинамики, что происходит с энергией в изолированной системе?",
    "options": [
        "Энергия может исчезать и появляться заново",
        "Энергия сохраняется, изменяя свою форму",
        "Энергия всегда переходит только в теплоту",
        "Энергия увеличивается при любых процессах"
    ],
    "correct_answer": "Энергия сохраняется, изменяя свою форму",
    "explanation": "Первый закон гласит, что энергия не создается и не уничтожается, а только переходит из одной формы в другую"
}}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📚 ПРИМЕР №2 (Вопрос на различие двух понятий):

❌ ФРАЗА ЛЕКТОРА В РАСШИФРОВКЕ:
"Тут важно не путать, ммм, скорость и ускорение. Это разные вещи, короче. Скорость — это как быстро движется, а ускорение — как быстро меняется скорость, понятно?"

✅ НОРМАЛИЗОВАННЫЙ ВОПРОС:
{{
    "question": "Чем отличается ускорение от скорости?",
    "options": [
        "Ускорение — это скорость изменения скорости",
        "Ускорение — это скорость, деленная на время",
        "Это одно и то же, просто разные названия",
        "Ускорение показывает пройденный путь"
    ],
    "correct_answer": "Ускорение — это скорость изменения скорости",
    "explanation": "Лектор объяснил: скорость — 'как быстро движется', а ускорение — 'как быстро меняется скорость'"
}}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📚 ПРИМЕР №3 (Вопрос на важную повторяемую идею):

❌ ФРАЗА ЛЕКТОРА В РАСШИФРОВКЕ:
"И ещё раз повторю, это очень важно, эээ... комплексные числа — это не просто математическая абстракция, они нужны для описания, как бы, волн, колебаний, всего такого..."

✅ НОРМАЛИЗОВАННЫЙ ВОПРОС:
{{
    "question": "Для описания каких физических явлений особенно важны комплексные числа?",
    "options": [
        "Только для квантовой механики",
        "Для волновых процессов и колебаний",
        "Только для электротехники",
        "Ни для каких, это чистая абстракция"
    ],
    "correct_answer": "Для волновых процессов и колебаний",
    "explanation": "Лектор специально подчеркнул, что комплексные числа важны для описания волн и колебаний"
}}

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎧 ИСХОДНЫЙ МАТЕРИАЛ (РАСШИФРОВКА АУДИОЛЕКЦИИ):
...
ПРАВИЛА ДЛЯ АУДИО-ТЕСТА:

1. ✅ Нормализуй речь до академического стиля
2. ✅ Убирай слова-паразиты
3. ✅ Выделяй ТО, ЧТО ЛЕКТОР ПОВТОРЯЛ (это важно!)
4. ❌ НЕ задавай вопросы про "на какой минуте"
5. ❌ НЕ копируй фразы с "эээ", "ну", "короче"
6. ❌ НЕ спрашивай про случайные примеры

СОЗДАЙ ТЕСТ из {num_questions} вопросов в формате JSON (как в примерах):

{{
    "test_title": "...",
    "source_type": "audio_lecture",
    "generation_method": "Few_Shot_Audio",
    "questions": [
        {{
            "id": 1,
            "question": "...",
            ...
            "correct_answer": "...",
            "explanation": "...",
            "original_lecture_phrase": "ключевая фраза из расшифровки (как было сказано)"
        }}
    ]
}}
"""
