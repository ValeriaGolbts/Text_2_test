...
class PipelineJSONProcessor:
    ...
    def select_chunks_accumulate(self, 
        ...
        """
        Отбирает чанки по стратегии "Ползучее накопление".
        
        Алгоритм:
        1. Фильтрует некачественные чанки
        2. Сортирует по информативности (score)
        3. Добавляет по одному, пока не заполнит fill_pct% контекстного окна
        4. Каждый следующий чанк добавляется ТОЛЬКО если влезает в лимит
        
        Это гарантирует, что промпт будет <= fill_pct% от контекстного окна.
        """
        ...
        print(f"\n{'='*60}")
        print(f"🐌 ОТБОР ЧАНКОВ: ПОЛЗУЧЕЕ НАКОПЛЕНИЕ (ДО {fill_pct}%)")
        print(f"{'='*60}")
        ...
        # === ШАГ 3: Расчёт лимита ===
        print(f"\n📐 ШАГ 3: Расчёт лимита контекстного окна...")
        ...
        SAFETY_MARGIN = 0.95                                # 95% для безопасности
        
        available_tokens = int(
            (MAX_CONTEXT - RESPONSE_RESERVE - PROMPT_OVERHEAD) * SAFETY_MARGIN
        )
        
        # Целевой лимит = fill_pct% от доступного
        target_token_limit = int(available_tokens * fill_pct / 100)
        ...
        print(f"  • Целевой лимит ({fill_pct}%): {target_token_limit:,} токенов")
        
        # === ШАГ 4: Ползучее накопление ===
        print(f"\n🐌 ШАГ 4: Ползучее накопление...")
        print(f"  Добавляем чанки по одному, пока не достигнем лимита...")
        ...
        for i, item in enumerate(passed_chunks, 1):
            # Проверяем: если добавим этот чанк, не превысим ли лимит?
            if selected_tokens + item['tokens'] > target_token_limit:
                # Пробуем добавить частично последний чанк
                remaining = target_token_limit - selected_tokens
                if remaining > item['tokens'] * 0.3:  # Если влезает >30% чанка
                    ...
                    chunk_count += 1
                    print(f"  • Чанк {i}: добавлен частично "
                          f"({item['tokens']} ток, score={item['score']:.1f}) "
                          f"[ЛИМИТ ДОСТИГНУТ]")
                else:
                    print(f"  • Чанк {i}: пропущен "
                          f"(нужно {item['tokens']} ток, осталось {remaining}) "
                          f"[ЛИМИТ ДОСТИГНУТ]")
                break
            
            # Добавляем чанк полностью
            selected.append(item['chunk'])
            selected_tokens += item['tokens']
            formulas_covered += item['formula_count']
            terms_covered.update(item['key_terms'])
            ...
            # Прогресс каждые 10 чанков или при достижении 50%/80%/90%
            progress_pct = selected_tokens / target_token_limit * 100
            if (chunk_count % 10 == 0 or 
                progress_pct >= 90 or 
                (progress_pct >= 80 and chunk_count % 5 == 0) or
                (progress_pct >= 50 and chunk_count == 1)):
                print(f"  • Чанк {i}: добавлен "
                      f"({item['tokens']} ток, score={item['score']:.1f}) | "
                      f"Всего: {selected_tokens:,}/{target_token_limit:,} ток "
                      f"({progress_pct:.0f}%)")
        
        # === ШАГ 5: Статистика ===
        fill_percentage = (selected_tokens / available_tokens * 100) if available_tokens else 0
        ...
        selection_info = {
            ...
            "strategy": "accumulate_to_90_percent"
        }
        
        print(f"\n{'='*60}")
        print(f"📊 РЕЗУЛЬТАТ ПОЛЗУЧЕГО НАКОПЛЕНИЯ")
        print(f"{'='*60}")
        print(f"  • Отобрано чанков: {len(selected)}")
        print(f"  • Заполнение: {selected_tokens:,} из {available_tokens:,} ток "
              f"({fill_percentage:.1f}%)")
        print(f"  • Целевой порог: {fill_pct}%")
        print(f"  • Средний score: {avg_score:.1f}")
        print(f"  • Покрытие формул: {formulas_pct:.1f}%")
        print(f"  • Покрытие терминов: {terms_pct:.1f}%")
        
        if fill_percentage < fill_pct - 5:
...
async def main():
    ...
    result = await processor.process_pipeline_json(
        json_path=json_file,
        num_questions=10,
        fill_pct=90  # Заполнить до 90% контекстного окна
    )
